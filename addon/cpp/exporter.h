...
{
	std::string inputImage;			///< Input filename
	...
	std::string copyrightText;		///< Copyright filename 
...
public:
	...
	virtual void WriteHeader()
	{
		...
		if (Param->bTitle)
		{
			WriteCommentLine((const char*)u8"██▀▀█▀▀██▀▀▀▀▀▀▀█▀▀█ ▄");
			WriteCommentLine((const char*)u8"██  ▀  █▄  ▀██▄ ▀ ▄█ ▄  ▄█▄█ ▄▀██");
			WriteCommentLine((const char*)u8"█  █ █  ▀▀  ▄█  █  █ ██ ██ █  ▀██");
			WriteCommentLine((const char*)u8"▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀          ▀▀");
		}
		...
		if (Param->bAddCopy)
		{
			std::ifstream file(Param->copyrightText);
			std::string strLine;
		...
		//WriteCommentLine(MSX::Format(" - Input file:     %s", Param->inputImage.c_str()));
