...
std::vector<unsigned char> decodeBase64(const std::string& encodedString)
{
	...
	ret.reserve((encodedString.size() * 3) / 4); // Riserva spazio approssimativo

	while (in_len-- && (encodedString[in_] != '=') && isBase64(encodedString[in_])) {
		char_array_4[i++] = static_cast<unsigned char>(encodedString[in_]);
		in_++;
		// Quando abbiamo accumulato 4 caratteri, convertiamoli in 3 byte
		if (i == 4) {
			for (int j = 0; j < 4; j++) {
	...
	// Gestione degli eventuali caratteri finali (padding)
	if (i > 0) {
		for (int j = i; j < 4; j++) {
			char_array_4[j] = 0;
		}
...
FIBITMAP* LoadImageFromBase64(const std::string& base64Data)
{
	// 1. Decodifica base64 in un buffer di byte
	std::vector<unsigned char> decodedData = decodeBase64(base64Data);
	if (decodedData.empty()) {
		// Se la decodifica fallisce o la stringa è vuota
		return nullptr;
	}

	// 2. Apri una memoria di FreeImage basata sui dati decodificati
	FIMEMORY* fiMem = FreeImage_OpenMemory(
		reinterpret_cast<BYTE*>(decodedData.data()),
		static_cast<DWORD>(decodedData.size())
	);
	if (!fiMem) {
		return nullptr;
	}

	// 3. Determina il formato (tipo di immagine) a partire dalla memoria
	FREE_IMAGE_FORMAT fif = FreeImage_GetFileTypeFromMemory(fiMem, 0);
	if (fif == FIF_UNKNOWN) {
		return nullptr;
	}

	// 4. Carica l’immagine in un FIBITMAP
	FIBITMAP* dib = FreeImage_LoadFromMemory(fif, fiMem, 0);

	// 5. Chiudi la memoria temporanea
	FreeImage_CloseMemory(fiMem);

	// Restituisci il bitmap (o nullptr se fallito)
	return dib;
}
...
/***/
bool ExportBitmap(ExportParameters * param, ExporterInterface * exp, std::string& exportedData)
{
	FIBITMAP *dib, *dib32;
	...
	dib = LoadImageFromBase64(param->inputImage.c_str()); // open and load the file using the default load option
	if (dib == NULL)
	{
		printf("Error: Fail to load %s\n", param->inputImage.c_str());
		return false;
	}
...
/***/
bool ExportGM1(ExportParameters* param, ExporterInterface* exp,std::string& exportedData)
{
...
/***/
bool ExportGM2(ExportParameters* param, ExporterInterface* exp, std::string &exportedData)
{
...
/***/
bool ExportSprite(ExportParameters* param, ExporterInterface* exp, std::string& exportedData)
{
	FIBITMAP* dib, * dib32;
	u32 sid = 0; // sprite id
...
/***/
bool ParseImage(ExportParameters* param, ExporterInterface* exp, std::string& exportdData)
{
	switch (param->mode)
	{
	default:
	case MODE_Bitmap:	return ExportBitmap(param, exp, exportdData);
	case MODE_GM1:		return ExportGM1(param, exp, exportdData);
	case MODE_GM2:		return ExportGM2(param, exp, exportdData);
	case MODE_Sprite:	return ExportSprite(param, exp, exportdData);
	};
}